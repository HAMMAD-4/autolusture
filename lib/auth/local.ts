...
export function makeSession(user:{id:string;role:Role;full_name:string}) { const value=Buffer.from(JSON.stringify({...user,exp:Date.now()+1000*60*60*12})).toString('base64url'); return `${value}.${sign(value)}`; }
export function readSession(raw:string|undefined): {id:string;role:Role;full_name:string}|null { if(!raw)return null; const [value,signature]=raw.split('.'); if(!value||!signature)return null; const expected=sign(value); if(signature.length!==expected.length||!timingSafeEqual(Buffer.from(signature),Buffer.from(expected)))return null; try { const payload=JSON.parse(Buffer.from(value,'base64url').toString()) as {id:string;role:Role;full_name:string;exp:number}; return payload.exp>Date.now()?payload:null; } catch { return null; } }
...
export async function currentUser() { const store=await cookies(); return readSession(store.get(cookieName)?.value); }
